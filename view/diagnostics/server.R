...
network <- function(site) {
  if (is.null(site)) {
    ...
    return('ch4')
  } else if (site %in% c('dbk', 'heb', 'imc', 'lgn', 'rpk', 'sug', 'sun')) {
    return('co2')
  } else {
    return(NULL)
  }
...
function(input, output, session) {
  ...
  observeEvent(r$refresh, {
    # showNotification('Generating timeseries...', duration = NULL,
                     # id = 'refreshing', closeButton = F)
    if (is.null(network(r$site))) return()
    if (input$dataset == 'Calibrated dataset') {
      opts <- switch(network(r$site),
                     'co2' = list(type = 'Tddddddc',
                                  col  = 'CO2d_ppm_cal',
                                  loc  = file.path('/projects/data', r$site,
                                                   'licor-6262', 'calibrated',
                                                   '%Y_%m_calibrated.dat')),
                     'ch4' = list(type = 'Tddddddddddddddc',
    ...
    dat <- format(seq(time_range[1], time_range[2], 3600*24), tz='UTC',
      ...
      lapply(function(f, col_types) {
        if (file.exists(f))
          read_csv(f, locale=locale(tz='UTC'), col_types=col_types)
      }, col_types=opts$type) %>%
      bind_rows() %>%
      ...
      if (input$dataset == 'Calibrated dataset') {
        if ('CH4d_ppm_cal' %in% names(r$data)) {
          dy <- dy %>%
            ...
            dyOptions(drawGrid=F, strokeWidth = 2)
        } else {
